...
### A weighted average of forecasts

.left-code[
```{r tourism-total-combn-2, eval=FALSE, echo=TRUE}
holiday_aus %>%
  filter(Quarter < yearquarter("2015 Q1")) %>%  
  model(
    ets_n = ETS(Trips ~ trend("N")),
...
.right-plot[
```{r tourism-total-combn-2-output, ref.label="tourism-total-combn-2", echo=FALSE, cache=TRUE, fig.height = 6}
```
]
...
holiday_aus %>% 
  ...
  mutate(
    combn = (ets_n + ets_a + arima)/3,
    wt_combn = combination_ensemble(ets_n, ets_a, arima, weights = "inv_var")
  ) %>% 
  accuracy() %>% #<<
  arrange(MASE) #<<
```
...
tourism_accuracy_reconciled <- fit %>% 
  reconcile(coherent = min_trace(wt_combn)) %>% #<<
  forecast(h = "3 years") %>% 
  accuracy(tourism_aggregated)
```
