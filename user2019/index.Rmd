...
```{r tourism-purpose-stl-mdl-deseas, echo = TRUE}
aus_travel_stl %>% 
  model(deseas = ETS(trend + remainder)) %>% #<<
  forecast(h = "3 years") %>% 
...
```{r tourism-purpose-stl-mdl-seas, echo = TRUE}
aus_travel_stl %>% 
  model(seas = SNAIVE(season_year)) %>% #<<
  forecast(h = "3 years") %>% 
  autoplot(aus_travel_stl)
```
...
```{r tourism-purpose-stl-mdl, echo = TRUE}
aus_travel_stl %>% 
  model(
    hybrid = ETS(trend + remainder) + SNAIVE(season_year)
  ) %>% 
  forecast(h = "3 years") %>% 
  autoplot(aus_travel_stl)
```
